...
{
    //-------------------------------------------------------------------------------------//
    //Investigated JSON strings that may gives difference between online and native parsing//
    //-------------------------------------------------------------------------------------//
    //by online paresr invalid JSON, but here - parsing success
    //"{{\"Status\":\"Json format is invalid. Please check JSON format\"}}";

    //"coolCSS" - attempt to parsing, by online invalid JSON
    //"{\"key\": \"<div class=\"coolCSS\">some text</div>\" }"

    //attempt to parsing for backslashes, by online valid JSON
    //"{\"testing\":\"1\t2\n3\"}"
    //but in this format OK
    //R"FF({"newLine":"aaa\n\t\tbbbb", "tab":"sss\t\n\rsss"})FF"

    //by online invalid JSON, here - parsing success
    //{"actor": ["id": "1","firstName": "Tom","lastName": "Cruise"]}
    //in both cases this is valid JSON
    //{"actor": [{"id": "1","firstName": "Tom","lastName": "Cruise"}]}

    //by online JSON invalid, here - parsing success
    //"{ "cols": [{"id":1, "label:Artikelname", "type:string"}] }"

    //by online JSON invalid, here - parsing success
    //"{\"val\":10., \"type\":\"double\"}"

    // the \/ combination gives a error unknown escape seqeuence, by online JSON valid
    //"{\"version\": 1494474840, \"preview_url\": \"https:\/\/www.youtube.com\/preview_image?filename=preview_001\u0026id=wgnh-CbdYns\u0026version=1494474840\u0026type=vi\", \"error\": \"\", \"success\": \"true\"}"

    //online valid JSON, here - attempt to parsing
    //{"some-dbl": -1e-4}

    //Duplicate key, names should be unique. Online warning invalid JSON, here - parsing success
    //"{"type":"double", "value":0.32, "type":"integer", "value":34}"

    //it's OK
    //{"type":"double", "value": 5.5}
    //but it is may cause counfusion, here parsing success, online - invalid JSON
    //{"type":"double", "value": 5,5}
    //{"value": "abcd","efgh"}

    /*
     //here - parsing success, but online invalid JSON
     {"list":[{"Summary":{"Id":"95","Name":"Martin"},{"Id":"96","Name":"Dennis"}}]}
    */
    //--------------------------------------------------------------------

    vector<string> strVect;
    strVect.push_back("{{\"Status\":\"The JSON string into {{}}\"}}");
    strVect.push_back("{\"Status\": \"Inside string \"exist double\" quotes\"}");
    strVect.push_back("{\"escape_character\":\"Escape sequences 1\t2\n3\"}");
    strVect.push_back("{\"person\": [\"ID\": \"1\",\"Name\": \"Tom\"]}");
    strVect.push_back("{\"cols\": [{\"id\":1, \"label:Artikelname\", \"type:string\"}] }");
    strVect.push_back("{\"val\":10., \"type\":\"double\"}");
    strVect.push_back("{\"type\":\"double\", \"value\": 1e-4}");
    strVect.push_back("{\"type\":\"double\", \"value\":0.3, \"type\":\"integer\", \"value\":3, \"Status\":\"Duplicate key\"}");
    strVect.push_back("{\"type\":\"double\", \"value\": 5,5}");
    strVect.push_back("{\"list\":[{\"Summary\":{\"Id\":95,\"Name\":\"Martin\"},{\"Id\":96,\"Name\":\"Dennis\"}}], \"status\":\"expecting_strging\"}");
    ...
    //some checked JSON strings in any formats
    //---------------------------------------------------------
    //"{\"hAxis\":{\"baselineColor\":\"transparent\", \"gridlines\":{\"color\":\"transparent\"}},\"vAxis\":{\"viewWindow\":{\"min\":0}},\"colors\":[\"#00bbde\",\"#fe6672\",\"#eeb058\"]}"
    //"{"fields": [{"type": "string", "name": "cname"}, {"type": "int", "name": "id"}] }"
    //"{\"url\":\"charting/Country.php?country=2&Type=0\", \"dataType\":\"json\", \"async\":false}"
    ...
    //"{"avatarNumber" : 2, "tagLine": "I'm from a test!"}"
    //"{\"actor\": [{\"id\": \"1\",\"firstName\": \"Tom\",\"lastName\": \"Cruise\"}]}"
