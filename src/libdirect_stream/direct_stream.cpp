...
public:
        ...
        , peerid(_peerid)
    {
        ...
        for (auto& existing_item : channel->streams)
        {
            ...
            insert_res_existing.first->second.push(packet(stream_join()));
            ...
            insert_res_new.first->second.push(packet(stream_join()));
    ...
    virtual ~direct_stream()
    {
        ...
        while (it != channel->streams.end())
        {
            ...
            existing_item.second.second[peerid].push(packet(stream_drop()));
            existing_item.second.first->wake();
...
stream_ptr construct_direct_stream(stream::peer_id const& peerid,
                                   event_handler& eh,
                                   direct_channel& channel)
{
    stream* p = new direct_stream(peerid, eh, channel);
    return stream_ptr(p);
}
}
